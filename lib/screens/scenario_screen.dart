...
// Use-case titles are derived from each scenario's own domain field rather
// than a hardcoded map, so a full exam-content pivot (new use cases,
// different names/count) needs no UI edits here.
String _useCaseTitle(Scenario scenario) => 'Use Case ${scenario.useCaseNumber} — ${scenario.domain}';
...
class _ScenarioListScreenState extends State<ScenarioListScreen> {
  ...
  Future<void> _load() async {
    ...
    if (!mounted) return;
    setState(() {
      _scenarios = scenarios..sort((a, b) => a.useCaseNumber.compareTo(b.useCaseNumber));
      _loading = false;
    });
  }
...
class _ScenarioDetailScreenState extends State<ScenarioDetailScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text(_useCaseTitle(scenario))),
      body: SafeArea(
        child: ListView(
