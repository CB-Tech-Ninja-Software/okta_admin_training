...
/// SQLite Database Manager for Directory Dash.
...
class AppDatabase {
  ...
  Future<void> init({Database? dbOverride, String dbName = 'directory_dash.db'}) async {
    if (dbOverride != null) {
      _db = dbOverride;
  ...
  Future<Database> _initDatabase({String dbName = 'directory_dash.db'}) async {
    ...
    return await openDatabase(
      ...
      onUpgrade: (db, oldVersion, newVersion) async {
        // All CREATE TABLE statements use IF NOT EXISTS, so re-running them
        // against an existing database safely adds only the tables that are
        // missing (e.g. reference_notes, added in version 2) without
  ...
  Future<void> _createTables(Database db) async {
    ...
    await db.execute('''
      CREATE TABLE IF NOT EXISTS scenarios (
        id TEXT PRIMARY KEY,
        domain TEXT NOT NULL,
        use_case_number INTEGER NOT NULL,
        title TEXT NOT NULL,
  ...
  Future<List<Question>> getAllQuestions({String? part, String? domain}) async {
    final db = await database;
    String? whereClause;
    List<dynamic> whereArgs = [];

    if (part != null && domain != null) {
      whereClause = 'part = ? AND domain = ?';
      whereArgs = [part, domain];
    } else if (part != null) {
      whereClause = 'part = ?';
      whereArgs = [part];
    } else if (domain != null) {
    ...
    final res = await db.query('questions', where: whereClause, whereArgs: whereArgs.isEmpty ? null : whereArgs);
  ...
  Future<List<Scenario>> getAllScenarios({int? useCaseNumber}) async {
    final db = await database;
    final res = await db.query(
      'scenarios',
      where: useCaseNumber != null ? 'use_case_number = ?' : null,
      whereArgs: useCaseNumber != null ? [useCaseNumber] : null,
    );
  ...
  Future<Map<String, Map<String, dynamic>>> getDomainStats() async {
    final db = await database;
    // Join attempts with questions to compute per-domain accuracy
