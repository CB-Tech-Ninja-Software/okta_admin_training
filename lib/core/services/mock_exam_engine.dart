...
class MockExamEngine {
  ...
  /// Starts a new Mock Exam session with domain-weighted questions matching the official exam count.
  Future<MockExamSession> startExam({
    List<Question>? questionsPool,
  }) async {
    final pool = questionsPool ?? await database.getAllQuestions(part: 'I');
  ...
  /// Selects domain-weighted questions based on Part I weights and mockExamQuestionCount.
  List<Question> _selectMockQuestions(List<Question> pool) {
    ...
    final weights = ExamConstants.part1DomainWeights;
    ...
    // Make sure sum of targets doesn't exceed targetCount
    ...
    // 2. Second pass: If still under targetCount, sample remaining from pool using weighted random rolls
  ...
  /// Retrieves past mock exam history.
