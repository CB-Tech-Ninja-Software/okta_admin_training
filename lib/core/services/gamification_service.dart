...
import 'package:directory_dash/core/models/badge.dart';
...
class GamificationService {
  ...
  /// Awards XP to the user and updates level.
  ...
  /// Increments quiz count, adds XP, and updates streak.
  ...
  Future<List<AppBadge>> checkAndAwardBadges(UserProfile profile) async {
    ...
    // 3. Streak Warrior Badge (3-day streak)
    ...
    // 4. Directory Legend Badge (Level 5+)
    if (!unlockedIds.contains('directory_legend') && profile.level >= 5) {
      unlockedIds.add('directory_legend');
      newlyAwarded.add(AppBadge.predefinedBadges.firstWhere((b) => b.id == 'directory_legend'));
    }

    // 5. Flashcard Fiend Badge (25+ flashcards reviewed)
